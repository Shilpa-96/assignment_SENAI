import React from "react";
import "./mainbody.css";
import { WiHumidity, WiRainMix } from "react-icons/wi";
import { PiWindFill } from "react-icons/pi";
import { BsSunFill } from "react-icons/bs";
import { FaTemperatureFull } from "react-icons/fa6";
import { FaCloudRain } from "react-icons/fa";
import BarChart from "./BarChart";
import AreaChart from "./AreaChart";

export default function Mainbody() {
  return (
    <div className="mainbody">
      <div className="header">
        <div className="header1" style={{ fontSize: "20px" }}>
          <p style={{ fontWeight: "700" }}>Welcome back Isabella!</p>
          <p>Check out today's weather information </p>
        </div>
        <div className="header2">
          <div style={{ fontWeight: "bold", fontSize: "20px" }}>...</div>
          <img
            src="data:image/jpeg;base64,"
            alt="profile"
          />
        </div>
      </div>
      <div className="chart">
        <AreaChart />
      </div>
      <h3 style={{ margin: "40px" }}>More details of today's weather</h3>
      <div className="cards">
        <div className="card card1">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>
              Humidity
            </p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <WiHumidity style={{ color: "white" }} />
            </div>
          </div>
          <span style={{ fontSize: "30px", fontWeight: "700" }}>
            82%
            <span
              style={{
                fontSize: "16px",
                fontWeight: "100",
                marginLeft: "10px",
              }}
            >
              bad
            </span>
          </span>
          <div className="gbn">
            <div className="skill-box">
              <span className="title">Good</span>
              <div className="skill-bar">
                <span className="skill-per good"></span>
              </div>
            </div>
            <div className="skill-box">
              <span className="title">normal</span>
              <div className="skill-bar">
                <span className="skill-per normal"></span>
              </div>
            </div>
            <div className="skill-box">
              <span className="title">bad</span>
              <div className="skill-bar">
                <span className="skill-per bad"></span>
              </div>
            </div>
          </div>
        </div>
        <div className="card">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>Wind</p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <PiWindFill style={{ color: "white" }} />
            </div>
          </div>
          <BarChart />
        </div>
        <div className="card card2">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>
              Percipitation
            </p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <WiRainMix style={{ color: "white" }} />
            </div>
          </div>
          <span style={{ fontSize: "30px", fontWeight: "700" }}>1.4 cm</span>
          <div className="preci">
            <div className="skill-box1">
              <span className="title">0</span>
              <div className="skill-bar">
                <span className="skill-per zero"></span>
              </div>
            </div>
            <div className="skill-box1">
              <span className="title">10</span>
              <div className="skill-bar">
                <span className="skill-per" style={{ width: "70%" }}></span>
              </div>
            </div>
            <div className="skill-box1">
              <span className="title">20</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">30</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">40</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">50</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">60</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">70</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">80</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box1">
              <span className="title">90</span>
              <div className="skill-bar"></div>
            </div>
          </div>
        </div>
        <div className="card card3">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>
              UV index
            </p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <BsSunFill style={{ color: "white" }} />
            </div>
          </div>
          <span style={{ fontSize: "30px", fontWeight: "700" }}>
            4
            <span
              style={{
                fontSize: "16px",
                fontWeight: "400",
                marginLeft: "5px",
              }}
            >
              medium
            </span>
          </span>
          <div className="preci">
            <div className="skill-box2">
              <span className="title">0-2</span>
              <div className="skill-bar">
                <span className="skill-per"></span>
              </div>
            </div>
            <div className="skill-box2">
              <span className="title">3-5</span>
              <div className="skill-bar">
                <span className="skill-per" style={{ width: "50%" }}></span>
              </div>
            </div>
            <div className="skill-box2">
              <span className="title">6-7</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box2">
              <span className="title">8-10</span>
              <div className="skill-bar"></div>
            </div>
            <div className="skill-box2">
              <span className="title">11+</span>
              <div className="skill-bar"></div>
            </div>
          </div>
        </div>
        <div className="card">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>
              Feels like
            </p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <FaTemperatureFull style={{ color: "white" }} />
            </div>
          </div>
          <span style={{ fontSize: "30px", fontWeight: "700" }}>30&deg;</span>
          <div className="preci">
            <div className="skill-box3">
              <div
                className="ttitle"
                style={{ display: "flex", justifyContent: "space-around" }}
              >
                <span className="title">0&deg;</span>
                <span className="title">25&deg;</span>
                <span className="title">50&deg;</span>
              </div>
              <div className="skill-bar">
                <span className="skill-per" style={{ width: "60%" }}></span>
              </div>
            </div>
          </div>
        </div>
        <div className="card">
          <div className="header">
            <p style={{ color: "rgb(74, 74, 74)", fontWeight: "700" }}>
              chance of rain
            </p>
            <div
              style={{
                width: "30px",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                height: "30px",
                backgroundColor: "#4070f4",
                marginTop: "20px",
                borderRadius: "10px",
              }}
            >
              <FaCloudRain style={{ color: "white" }} />
            </div>
          </div>
          <span style={{ fontSize: "30px", fontWeight: "700" }}>42%</span>
          <div className="preci">
            <div className="skill-box4">
              <div
                className="ttitle"
                style={{ display: "flex", justifyContent: "space-around" }}
              >
                <span className="title">0%</span>
                <span className="title">25%</span>
                <span className="title">50%</span>
                <span className="title">75%</span>
                <span className="title">100%</span>
              </div>
              <div className="skill-bar">
                <span className="skill-per" style={{ width: "43%" }}></span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
